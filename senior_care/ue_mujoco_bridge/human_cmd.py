"""Human state frame transmitted over ZMQ from MuJoCo to UE."""
...
@dataclass
class HumanFrame:
    """One human body's state in a single simulation step.

    Same wire format as RobotFrame but kept separate so each type
    can evolve independently (e.g. adding SMPL pose parameters).

    Attributes
    ----------
    root_position : [x, y, z] in meters (MuJoCo frame).
    root_orientation : [w, x, y, z] quaternion (MuJoCo frame).
    joints : {joint_name: position_rad}.
    ...
    @classmethod
    def from_dict(cls, d: dict) -> HumanFrame:
        return cls(
